...
- [ ] Rspecが落ちないこと
- [ ] Rubocopでエラーが出ないこと
- [ ] ESLint( `npm run eslint`)でエラーが出ないこと
- [ ] 各資料のリンク貼ったか
  - チケット、テストケース、リリースノート、関連PR、wiki（追記した場合）
- [ ] 仕様の意図や背景が分かるようになっているか
  <!-- コーディング、コメントアウト、wikiに記載、のいずれかで仕様の意図や背景が分かるようになっているか -->


## Reviewer's Checklist
- [ ] 各資料の添付・リンク等があるか
  - チケット、テストケース、リリースノート、関連PR、wiki（追記した場合）、rubocop（スクショ）, eslint（スクショ）, rspec（スクショ）
- [ ] 各項目が簡潔に書かれているか
- [ ] 要件を満たしているか
- [ ] 意図した仕様か、正常に動いているか
- [ ] 仕様はこれがあるべき姿か
- [ ] 保守性・可読性
  - コード規約、その言語らしい書き方ができているか、設計の原理原則に則っているか
  - 読みにくくないか、命名など意図が伝わるものか
  - 意図や背景が分かるコーディングか。分からない部分にはコメント（もしくはwikiに記載してリンク）を載せているか
- [ ] 影響範囲
  - 範囲を把握し、バグの可能性を考慮できているか

## 注意事項
- force push 禁止です。
- チケットURL、リリースノート、テストケースがない場合、どんな簡易な修正でもmergeしません。
- esg merge等は特に開発経緯や開発内容の記載をお願いします。
- frontendでlib配下を更新した場合は`npm run build-prod-sdk`をし、
  production用にコンパイルしたpublic/ukokkei-sdk.jsなどを一緒にpushしてください。

### コメントアウトのルール
- 意図や背景がコードから読み取れない場合、分かるように補足する
  - 記述量などによりコード上に記載することが憚られる場合はwiki（仕様/仕様の背景/）に記載してリンクを載せる
- 外部連携で仕様書を読まないと分からないような仕様の場合、簡潔に説明する
  理解に必要な仕様書やチケットのリンクがあれば載せる
- 今後改修予定、改修すべきもの、不足している機能や対応（`# TODO: `）
- 修正すべき壊れたコード（`# FIXME: `）
- パフォーマンスに影響を与える最適化すべき箇所（`# OPTIMIZE: `）
